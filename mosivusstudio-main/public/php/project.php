...
if ($name === '' || $email === '' || $description === '') {
    jsonResponse(['status' => 'error', 'message' => 'Name, email, and project description are required.'], 400);
}
if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    jsonResponse(['status' => 'error', 'message' => 'Invalid email address.'], 400);
}
if (strlen($name) > 200 || strlen($email) > 255 || strlen($description) > 10000) {
...
try {
    ...
    $stmt = $db->prepare("
        INSERT INTO project_submissions (name, email, phone, role, source, budget, timeline, description)
        VALUES (:name, :email, :phone, :role, :source, :budget, :timeline, :description)
    ");
    $stmt->execute([
        ':name'        => $name,
        ':email'       => $email,
    ...
    error_log('Project form DB error: ' . $e->getMessage());
